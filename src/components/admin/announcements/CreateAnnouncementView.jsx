...
const CreateAnnouncementView = ({ editingAnnouncement, onCreated, onCancel }) => {
  ...
  return (
    <div>
      <h1 className="font-bold text-2xl mb-5"> {isEditMode ? "EDIT ANNOUNCEMENT" : "CREATE ANNOUNCEMENT"}</h1>
      <div className="section" style={{ padding: "24px" }}      >
        <form onSubmit={handleSubmit}>
          <div className="form-grid">
            <div className="form-field">
              <label className="form-label">Titl</label>
              <input  type="text" className="form-input" placeholder="Announcement title"value={title}onChange={(e) =>
          ...
          <div className="form-field">
            ...
            <textarea rows={6} className="form-textarea"placeholder="Write the announcement details..."value={message}onChange={(e) =>
          ...
          <div className="form-grid">
            <div className="form-field">
              ...
              <input type="file" accept="image/jpeg"className="form-input"onChange={handleImageChange}/>
              {imageError && (
                <span style={{color: "red",fontSize: ".8rem",}}>{imageError}</span>
              )}
              {isEditMode && existingImageUrl && !imageFile && !removeImage && (
                <div  style={{ marginTop: "10px",display: "flex", alignItems: "center",gap: "12px", }}>
                  <img src={existingImageUrl}alt="Current"style={{width: "56px",height: "56px",objectFit: "cover",borderRadius: "6px",border: "1px solid #e5e7eb",
                    }}/>
                  <button
                    type="button" onClick={() => setRemoveImage(true)} style={{ background: "none", border: "none", color: "#dc2626", cursor: "pointer",fontSize: ".8rem",
                      padding: 0,
              ...
              {isEditMode && removeImage && (
                <div
                  style={{marginTop: "10px", display: "flex", alignItems: "center", gap: "10px",fontSize: ".8rem",
                    color: "#6b7280", }} >
            ...
            <div className="form-field">
              ...
              <input type="url" className="form-input" placeholder="https://..." value={sourceUrl} onChange={(e) =>
          ...
          {linkPreview && (
            <div style={{ marginTop: "20px", border: "1px solid #e5e7eb", borderRadius: "10px", padding: "16px", display: "flex",
                gap: "16px", alignItems: "center", background: "#fafafa",}}>
              {linkPreview.image && (
                <img src={linkPreview.image} alt="Preview"style={{
                    width: "120px",height: "90px",objectFit: "cover", borderRadius: "8px", flexShrink: 0, }} /> )}
              <div>
                <h5  style={{margin: "0 0 6px",fontWeight: 600,}}>
                  {linkPreview.title}
                </h5>
                {linkPreview.site && (
          ...
          <div style={{ display: "flex", justifyContent: "flex-end", gap: "10px", marginTop: "25px",}}>
            ...
            <button type="submit" className="button button-primary" disabled={submitting}>
              {submitting ? isEditMode  ? "Updating..."  : "Saving..."  : isEditMode  ? "Update Announcement"  : "Post Announcement"}
            </button>
          </div>
        </form>
      </div>
